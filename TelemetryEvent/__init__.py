    ...
    try:
        ...
        try:
            ...
            if 'NEURAL_NETWORK' in data_dict.keys():
                ...
                logging.info('count string: ' + counts_string)
                
                # Percept에서 IoT Hub로 보낸 Telemetry에서 사람 수 추출
                detect_cnt = json.loads(counts_string)

                # people_cnt가 0명인 경우 int 0으로 처리
                if len(detect_cnt) == 0:
                    people_cnt = 0
                else:
                    people_cnt = detect_cnt['person']

                # percept에서 counting 된 사람 수 log로 출력
                logging.info('people_cnt: ' + str(people_cnt))
